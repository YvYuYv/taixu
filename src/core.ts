/**
 * @taixu/core 打包入口：框架核心（8 服务 + 沙箱 + 主题 + 水合 + PII + 时间旅行 + AMD 命名空间）。
 * 适配器拆分在独立包（@taixu/adapter-vue3 / adapter-vue2 / adapter-react / adapter-angular），
 * 避免核心用户被迫安装各技术栈依赖。src/index.ts 为全量聚合入口（开发/测试用）。
 */
